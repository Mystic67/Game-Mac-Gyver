...
class Sprite:
    ...
    def display(self, window):
        ''' This method is needed to display the sprite on at position or refresh
        at position after move.'''
    ...
    def __init_position(self):
        '''This method initialise the position with random or given position. '''
        if self.__init_pos == "random":
            type(self).object_create += 1
            #print("objet créer {}".format(self.create_object))
            rand_pos = random.choice([pos for (pos, val) in self.__map.items() if val == constants.PATH])
            self.__x = rand_pos.get_x()
            self.__y = rand_pos.get_y()

        else:
            # if the position is given in param. The position (x,y) is stored in list whith list comprehention,
            # also need to accès with index 0
            position = [pos for (pos, val) in self.__map.items() if val == self.__init_pos]
